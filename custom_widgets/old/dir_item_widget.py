...
from global_enums import CustomRoles, TaskState, ItemTypes 
...
class DirItemWidget:
    def __init__(self):
        ...
        self.remove.addStateIcon(
            WidgetState.CLICKED, QPixmap(wrkdir + "res/icons/remove_shunken.png")
        )
        self.render = IconButton(
            QPixmap(wrkdir + "res/icons/render.png"), WidgetState.ENABLED
        )
        self.render.addStateIcon(
            WidgetState.CLICKED, QPixmap(wrkdir + "res/icons/render_shunken.png")
        )

        # Enable and Dependent checkboxes
        self.enable = IconButton(
            QPixmap(wrkdir + "res/icons/enable_on.png"), WidgetState.ENABLED
        )
        self.enable.addStateIcon(
            WidgetState.DISABLED, QPixmap(wrkdir + "res/icons/enable_off.png")
        )
        self.dependent = IconButton(
            QPixmap(wrkdir + "res/icons/dependent_on.png"), WidgetState.ENABLED
        )
        self.dependent.addStateIcon(
            WidgetState.DISABLED, QPixmap(wrkdir + "res/icons/dependent_off.png")
        )
        self.name = TextItem(text_size=12)

        # State icon
        self.state = IconButton(
            QPixmap(wrkdir + "res/icons/StateIcon_waiting.png"), TaskState.WAITING
        )
        self.state.addStateIcon(
            TaskState.RENDERING, QPixmap(wrkdir + "res/icons/StateIcon_rendering.png")
        )
        self.state.addStateIcon(
            TaskState.FAILED, QPixmap(wrkdir + "res/icons/StateIcon_failed.png")
        )
        self.state.addStateIcon(
            TaskState.SUCCESSFUL, QPixmap(wrkdir + "res/icons/StateIcon_successful.png")
        )
        ...
        # Handle mouse button release
            ...
            if self.enable.contains(pos):
                ...
                if dependent_state & WidgetState.ENABLED:
                    model.setData(
                        index, WidgetState.DISABLED, CustomRoles.DependentState
                    )
                elif dependent_state & WidgetState.DISABLED:
                    model.setData(
                        index, WidgetState.ENABLED, CustomRoles.DependentState
                    )
