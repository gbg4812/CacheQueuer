...
class TaskItemWidget():
    def __init__(self):
        ...
        self.render.addStateIcon(WidgetState.DISABLED, QPixmap(
            wrkdir + "res/icons/render_disabled.png"))

        self.enable = IconButton(
            QPixmap(wrkdir + "res/icons/enable_on.png"), WidgetState.ENABLED, paint_rect=True)
        self.enable.addStateIcon(WidgetState.DISABLED, QPixmap(
            wrkdir + "res/icons/enable_off.png"))
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        if (option.state & QStyle.State_Selected):
            TaskItemWidget.paintBackground(
                painter, rect, Border1, Primary3_selected)
        else:
